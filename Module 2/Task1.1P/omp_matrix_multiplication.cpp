...
#pragma omp parallel for collapse(2) // the collapse(2) tells OpenMP to treat
                                     // the two nested loops as a single loop.
...
int main() {
  ...
  try {
    ...
    outputFile << std::endl;
    outputFile << "Resulting matrix (OMP Parallelism): " << std::endl;
